...
export function startConversation(
  profile: SessionSnapshot["client_profile"],
): Promise<ConversationStarted> {
  return request<ConversationStarted>("/sessions", {
    method: "POST",
...
export function sessionEventStreamUrl(
  sessionId: string,
  afterSequence: number,
): string {
  return `${API_ROOT}/sessions/${sessionId}/events/stream?after_sequence=${afterSequence}`;
}
...
export function getKnowledgeChunks(filters: {
  ...
  params.set("limit", "50");
  return request<KnowledgeChunkPage>(
    `/inspection/knowledge/chunks?${params.toString()}`,
  );
}
